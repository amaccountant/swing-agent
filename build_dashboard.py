# build_dashboard.py — SPA dashboard (Drop 1): sidebar/bottom-nav, Overview + Ideas views,
# dark glassmorphism bento, theme toggle, language dropdown, preference memory.
# Data layer unchanged. All times CET. Research/education. NOT financial advice.
...
def main():
    ...
    payload = {
        ...
        "eurinr": ind.get("eurinr","?"), "picks": picks_all,
        "perf": {"deHit": dh, "deTot": dt, "inHit": ih, "inTot": it,
                 "deSeries": perf_series("performance_log.csv"),
                 "inSeries": perf_series("performance_log_in.csv")},
...
[data-theme="light"]{--bg:#eef1f7;--bg2:#ffffff;--glass:rgba(0,0,0,.03);--line:rgba(0,0,0,.10);--txt:#141b2b;--mut:#5b6577;--acc:#2b5cff;--acc2:#7a3cff;--good:#12a150;--bad:#e03e52;--warn:#b7791f}
*{box-sizing:border-box}html,body{margin:0;height:100%}
body{font-family:-apple-system,'Segoe UI',Roboto,Arial,sans-serif;background:radial-gradient(1200px 600px at 80% -10%,rgba(108,140,255,.12),transparent),radial-gradient(900px 500px at -10% 20%,rgba(154,108,255,.10),transparent),var(--bg);color:var(--txt);transition:background .4s,color .4s}
...
.logo{display:flex;align-items:center;gap:10px;padding:6px 8px 16px;font-weight:800;font-size:18px}
...
.nav b .ic{font-size:18px;width:22px;text-align:center}
.nav b:hover{background:var(--glass);color:var(--txt)}
...
.vt{font-size:18px;font-weight:700}
...
.glass{background:var(--glass);border:1px solid var(--line);border-radius:18px;padding:18px;backdrop-filter:blur(8px);transition:transform .25s,box-shadow .25s}
...
.hero .hl{color:var(--mut);margin-top:8px;font-size:14px}
.kpi .k{font-size:30px;font-weight:800;color:var(--acc)}.kpi .l{color:var(--mut);font-size:12px;margin-top:4px}
...
.pcard{background:var(--glass);border:1px solid var(--line);border-radius:18px;padding:16px;backdrop-filter:blur(8px);transition:transform .25s,box-shadow .25s}
...
.pchead{display:flex;justify-content:space-between;gap:10px}
.pchead h3{margin:0;font-size:16px}.ticker{color:var(--acc);font-size:12px;font-weight:600}
...
.kv{background:var(--bg2);border:1px solid var(--line);border-radius:10px;padding:8px;font-size:12px}
.kv span{color:var(--mut);font-size:11px;display:block}.kv b{font-size:14px}
...
.guide ol{margin:8px 0 0 18px;font-size:14px}.guide li{margin:6px 0}
...
@media(max-width:820px){
  ...
  .bento{grid-template-columns:repeat(2,1fr)}.hero{grid-column:span 2}.span2{grid-column:span 2}.span4{grid-column:span 2}
  ...
  .bottomnav b{display:flex;flex-direction:column;align-items:center;gap:2px;color:var(--mut);font-size:10px;font-weight:600;cursor:pointer;padding:4px 8px;border-radius:10px}
  .bottomnav b.active{color:var(--acc)}.bottomnav b .ic{font-size:20px}
}
</style></head><body>
<div class="app">
  ...
  <div class="main">
    <div class="topbar">
      <div class="vt" id="viewTitle">Overview</div>
      <div class="ctrls">
        <div id="gt"></div>
    ...
    <div class="content">
      ...
      <section class="view active" id="v-overview">
        ...
        <div class="bento">
          <div class="glass hero"><div class="hn num" id="heroNum">0</div><div class="hl">Actionable ideas today — hand-picked from both markets, already filtered for trading costs.</div></div>
        ...
        <div class="glass guide" style="margin-top:14px"><h3 style="margin:0 0 6px">🧭 How to read this</h3><ol>
          <li>Open <b>Ideas</b> (left/bottom nav) for today’s picks.</li>
          <li>Green <b>✓ ACTIONABLE</b> = worth considering; <b>WATCH/SKIP</b> = don’t trade today.</li>
          <li>Confidence dial, Risk/Reward bar and the stop→buy→now→target scale tell the story at a glance.</li>
          <li>Always confirm the <b>live price</b> in your broker — data is ~15 min delayed.</li>
        </ol></div>
      </section>
      ...
      <section class="view" id="v-analytics"><div class="soon">📊 Analytics — arriving in Drop 2 (accuracy trends, hit-rate donuts, deviation charts).</div></section>
      <section class="view" id="v-watchlist"><div class="soon">📋 Watchlist — arriving in Drop 2 (full analysed universe).</div></section>
      <section class="view" id="v-learnings"><div class="soon">🧠 Learnings — arriving in Drop 2 (strategy-memory timeline).</div></section>
      <section class="view" id="v-about"><div class="soon">ℹ️ About &amp; methodology — arriving in Drop 2.</div></section>
    </div>
  </div>
</div>
...
function loadPrefs(){const t=localStorage.getItem('sa_theme');document.documentElement.setAttribute('data-theme',t||'dark');window._filter=localStorage.getItem('sa_filter')||'all';const s=localStorage.getItem('sa_sort')||'score';document.getElementById('sortsel').value=s;if(localStorage.getItem('sa_side')==='1')document.getElementById('side').classList.add('collapsed');}
function toggleTheme(){const c=document.documentElement.getAttribute('data-theme');const n=c==='light'?'dark':'light';document.documentElement.setAttribute('data-theme',n);localStorage.setItem('sa_theme',n);document.getElementById('themeBtn').textContent=n==='light'?'🌙':'☀️';}
...
function applyFilters(){const q=(document.getElementById('search').value||'').toLowerCase();const s=document.getElementById('sortsel').value;localStorage.setItem('sa_sort',s);let a=[...DATA.picks];a.sort((x,y)=>s==='reward'?y.tgt_move_pct-x.tgt_move_pct:s==='move'?y.atr_pct-x.atr_pct:y.score-x.score);const f=window._filter;a=a.filter(p=>{if(f==='actionable'&&!p.actionable)return false;if(f==='de'&&p.india)return false;if(f==='in'&&!p.india)return false;if(f==='high'&&p.conf!=='High')return false;if(q&&!(p.name+' '+p.ticker).toLowerCase().includes(q))return false;return true;});document.getElementById('cards').innerHTML=a.length?a.map(card).join(''):'<div class="pcard skp"><p class="plain">No ideas match. Sitting in cash is a valid, cost-free choice.</p></div>';}
function countUp(el,to){let n=0;const step=Math.max(1,Math.ceil(to/24));const t=setInterval(()=>{n+=step;if(n>=to){n=to;clearInterval(t);}el.textContent=n;},22);}
function spotlight(){const acts=DATA.picks.filter(p=>p.actionable).sort((a,b)=>b.score-a.score);const el=document.getElementById('spotlight');if(!acts.length){el.innerHTML='<b>🎯 Idea of the day</b><p class="plain" style="margin:8px 0 0">No actionable idea today. Cash is a valid position.</p>';return;}const p=acts[0];el.innerHTML=`<b>🎯 Idea of the day</b><h3 style="margin:8px 0 2px">${p.name} <span class="ticker" translate="no">${p.ticker}</span></h3><p class="plain" style="margin:0">${p.actionable?'✓ Actionable':''} · Confidence ${p.conf} · target ${p.cur}${p.target} (~${p.tgt_move_pct}%)</p><p class="plain" style="margin:6px 0 0;color:var(--mut)">Open <b>Ideas</b> for full details.</p>`;}
function miniAcc(){const all=[...DATA.perf.deSeries,...DATA.perf.inSeries].sort((a,b)=>a.date.localeCompare(b.date));const L=[],V=[];let h=0,t=0;all.forEach(r=>{t++;h+=r.hit;L.push(r.date);V.push(Math.round(100*h/t));});const c=document.getElementById('miniAcc');if(!L.length){c.replaceWith(Object.assign(document.createElement('div'),{className:'soon',textContent:'Accuracy chart appears after first reviews.'}));return;}new Chart(c,{type:'line',data:{labels:L,datasets:[{label:'Cumulative accuracy %',data:V,borderColor:'#6c8cff',backgroundColor:'rgba(108,140,255,.15)',fill:true,tension:.3,pointRadius:2}]},options:{plugins:{legend:{labels:{color:getComputedStyle(document.body).color}}},scales:{y:{min:0,max:100,ticks:{color:'#8b95a7'}},x:{ticks:{color:'#8b95a7'}}}}});}
...
function init(){loadPrefs();document.getElementById('themeBtn').textContent=document.documentElement.getAttribute('data-theme')==='light'?'🌙':'☀️';document.querySelectorAll('#nav b,#bnav b').forEach(b=>b.onclick=()=>nav(b.dataset.v));const ah=DATA.perf.deHit+DATA.perf.inHit,at=DATA.perf.deTot+DATA.perf.inTot;countUp(document.getElementById('heroNum'),DATA.actionableToday);countUp(document.getElementById('kAnalysed'),DATA.picks.length);document.getElementById('kAccuracy').textContent=at?Math.round(100*ah/at)+'%':'—';countUp(document.getElementById('kDE'),DATA.picks.filter(p=>!p.india).length);countUp(document.getElementById('kIN'),DATA.picks.filter(p=>p.india).length);document.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active',c.dataset.f===window._filter));applyFilters();spotlight();miniAcc();stale();const s=document.createElement('script');s.src='https://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit';document.head.appendChild(s);}
document.addEventListener('DOMContentLoaded',init);
    ...
    print("SPA dashboard (Drop 1) written: " + str(len(picks_all)) + " picks")
