...
# Same tradability scoring, INR pool, 1.5%-bar-appropriate. Research/education. NOT financial advice.
...
POOL_INR = 20000.0
KEEP = 12
MIN_ATR_PCT = 1.2   # India names move a bit more; slightly lower floor

# Broad liquid NSE universe (large + high-beta mid caps). Edit freely.
UNIVERSE = {
...
def score(ticker):
    try:
        ...
        h=h[["High","Low","Close","Volume"]].dropna(); h=h[h["Close"]>0]
        ...
        price=c[-1]
        if price>POOL_INR: return None
        trs=[]
        for i in range(-14,0):
            trs.append(max(hi[i]-lo[i],abs(hi[i]-c[i-1]),abs(lo[i]-c[i-1])))
        atrp=(sum(trs)/14/price)*100
        if atrp<MIN_ATR_PCT: return None
        avgvol=sum(v[-20:])/20; turnover=avgvol*price
        sma5=sum(c[-5:])/5; sma20=sum(c[-20:])/20; trend=1 if sma5>sma20 else 0
        g=l=0.0
        for i in range(-14,0):
        ...
        print("  [skip] "+ticker+": "+str(e)); return None

def main():
    ranked=[]
    for t,name in UNIVERSE.items():
        r=score(t)
        if r: r["name"]=name; ranked.append(r)
    ranked.sort(key=lambda x:x["score"],reverse=True)
    top=ranked[:KEEP]
    with open("watchlist_in.csv","w",newline="") as f:
        w=csv.writer(f); w.writerow(["ticker","name","note"])
        for r in top:
            w.writerow([r["ticker"],r["name"],"auto: ATR "+str(r["atrp"])+"% score "+str(r["score"])])
    print("India watchlist refreshed: "+str(len(top
